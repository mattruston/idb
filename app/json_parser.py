...
class Parser:
                ...
                if 'games' in platform:
                    for game in platform['games']:
                        if game in self.game_dict:
                            ...
                            for developer in game_row.developers:
                                if developer not in new_row.developers:
                                    new_row.developers.append(developer)
                            if game_row.rating:
                                rating_sum += game_row.rating
                                rating_count += 1 
                if rating_sum:
    ...
    def parse_characters(self):
        with open('data/characters.json') as character_data:
            ...
            for character in data:
                ...
                if 'image' in character:
                    d['image_url'] = character['image']['original_url'].strip('\\')
                    d['thumb_url'] = character['image']['medium_url'].strip('\\')
                if 'gender' in character:
                    if character['gender'] not in (0, 1, 2):
                        d['gender'] = self.gender[0]
                    else:
        ...
        for game, related in self.related_games.items():
            ...
            for elem in related:
                if elem in self.game_dict:
                    results.append(app.models.Game.query.filter(app.models.Game.name == self.game_dict[elem]['name']).first().game_id)
            app.models.Game.query.filter(app.models.Game.name == self.game_dict[game]['name']).first().related_game_ids = results
            app.models.db.session.commit()